...
See FAQ to figure out why this was done.
...
The second easiest thing to do is to do `python setup.py install` in the `creds3` directory.
...
Once credentials are in place, run `creds3 setup`. This will create the DDB table needed for credential storage.
...
## Usage
```
usage: creds3 [-h] [-r REGION] [-t TABLE] {delete,get,getall,list,put,setup} ...

A credential/secret storage system

delete
    usage: creds3 delete [-h] [-r REGION] [-t TABLE] credential

    positional arguments:
      credential  the name of the credential to delete

get
    usage: creds3 get [-h] [-r REGION] [-t TABLE] [-k KEY] [-n] [-v VERSION]
...
getall
    usage: creds3 getall [-h] [-r REGION] [-t TABLE] [-v VERSION] [-f {json,yaml,csv}]
                            [context [context ...]]
...
list
    usage: creds3 list [-h] [-r REGION] [-t TABLE]
...
setup
    usage: creds3 setup [-h] [-r REGION] [-t TABLE]
...
### Setup Permissions
In order to run `creds3 setup`, you will also need to be able to perform the following DDB operations:
```
{
    "Version": "2012-10-17",
    "Statement": [
...
tl;dr: If you are using less than 25 reads/sec and 25 writes per second on DDB today, it will cost ~$1/month to use creds3.
...
If you are using creds3 heavily and need to increase the provisioned reads/writes, you may incur additional charges. You can estimate your bill using the AWS Simple Monthly Calculator (http://calculator.s3.amazonaws.com/index.html#s=S3).

### 4. Why S3 for the credential store? Why not DynamoDB?
While DDB fits the application really well, there might be places where you are forced to use S3 for storing your secrets (enterprises come to mind). This program is a total rip-off of the [Credstash](https://github.com/fugue/credstash) with the DynamoDB replaced by S3 to serve that particular use case.
